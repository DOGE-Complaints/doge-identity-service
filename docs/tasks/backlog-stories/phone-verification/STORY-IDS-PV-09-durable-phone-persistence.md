...
- **Зависит от:** [PV-03](STORY-IDS-PV-03-otp-engine-session.md) (OTP-движок + контракт сессии верификации), [PV-04](STORY-IDS-PV-04-profile-flag-migration.md) (phone-колонки профиля + миграция). **Прецедент дизайна:** [STORY-IDS-OAUTH-03](../oauth/STORY-IDS-OAUTH-03-persistent-token-store-supabase.md) (🟢 Done — durable Supabase-персистентность по той же модели «контракт-парити с in-memory + DI-переключатель по backend»).
...
OAuth уже прошёл этот путь: его короткоживущее состояние выдачи перенесено в Supabase Postgres при `DB_BACKEND=supabase`, а demo/offline остаётся in-memory ([OAUTH-03](../oauth/STORY-IDS-OAUTH-03-persistent-token-store-supabase.md), 🟢). Phone-гейт должен получить **ту же durability** по тому же паттерну.
...
## Вне scope
- **IP/UA-хэширование содержимого аудита** — это [STORY-IDS-SEC-02](../security-hardening/STORY-IDS-SEC-02-audit-ip-ua-hashing.md) (G-2b) в пакете security-hardening. Здесь — **durable хранилище**; проброс+хэш request-контекста — там. Разделение зафиксировано в [`EPIC-IDS-SEC`](../security-hardening/EPIC-IDS-SEC.md) (§Состав, §Порядок: «durable-хранилище — там, проброс+хэш IP/UA — здесь»).
- **Access-token statelessness** — не меняется (тема OAuth; stateless HS256-JWT и так переживает редеплой).
- **Rate-limiting** — [STORY-IDS-SEC-01](../security-hardening/STORY-IDS-SEC-01-rate-limiting.md) (G-1).
...
## Парадигма-якорь
[`identity-backend-full-audit-2026-06-24.md`](../../../analysis/identity-backend-full-audit-2026-06-24.md) §3 (G-2a/G-3 + bootstrap), [STORY-IDS-OAUTH-03](../oauth/STORY-IDS-OAUTH-03-persistent-token-store-supabase.md) (прецедент durable Supabase-персистентности), [`runtime-docs/04-security.md`](../../../runtime-docs/04-security.md) (durable аудит, session-binding), [`08-supabase-migrations`](../../../requirements/08-supabase-migrations.md).
