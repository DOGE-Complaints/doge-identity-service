...
- **Решения (интервью 2026-07-04):** [`identity-mvp-hardening-interview`](../../../analysis/identity-mvp-hardening-interview-2026-07-04.md) — **D-1** JWKS-only, **D-2** гейт по реальному URL, **D-5** удалить `SUPABASE_JWT_SECRET`.
- **Зависит от:** [SEC-03](STORY-IDS-SEC-03-jwt-validation-hardening.md) (🟢 `aud`+клеймы — тот же валидатор; SEC-03 вынес «RS256/JWKS» в отдельную тему — это она)
...
- [ ] `grep -rn "_validate_hs256\|OctKey\|test-secret-for-demo\|demo\.local\|supabase_jwt_secret\|SUPABASE_JWT_SECRET" src/` → **пусто** (HS256/секрет Supabase-валидатора и sentinel удалены). *(HS256 в `oauth/access_token_jwt.py` сохранён — вне этого grep.)*
- [ ] Валидатор принимает `JwksCache` снаружи (DI из `providers.py`), собственный `httpx.Client` не создаёт; клиент переиспользуется/закрывается (W2). `except` сужен (W6).
- [ ] JWKS-гейт по реальному `supabase_url`; при пустом URL — fail-closed (нет HS256-фолбэка).
- [ ] `SUPABASE_JWT_SECRET` удалён из `schema.py`, `providers.py`, `.env.example`; pilot-required без него; `test_config_schema.py` обновлён.
- [ ] Тест-харнес переведён на ES256+мок-JWKS: `grep -rn "alg\": \"HS256\"" tests/` не встречается для Supabase-токенов (OAuth-тесты access-token — отдельно); все 14 файлов зелёные.
- [ ] Тесты валидатора ES256 (happy, unknown-kid→refresh, kid-not-found, alg-not-allowed, `alg=none`/`HS256` reject, jwks-unavailable, клеймы).
- [ ] Доки [`09`](../../../requirements/09-supabase-jwt-validation.md)/[`04-security.md`](../../../runtime-docs/04-security.md)/[`07`](../../../requirements/07-env-configuration-spec.md)/runbooks приведены к **JWKS/ES256-only**; `test_supabase_runbook_docs.py` синхронизирован.
- [ ] Полная offline-сюита зелёная: `.venv/bin/python -m pytest -q -m "not live_integration"` (счётчик изменится: HS256-минты заменены ES256).
