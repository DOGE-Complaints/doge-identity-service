...
- **Status:** 🟢 Done (синхр. 2026-06-26: код построен — [`security/session_secret.py`](../../../../src/core/security/session_secret.py) `FernetSessionSecretBox`; совпадает с `EPIC-IDS-EID`)
- **Источник:** аудит [`authentigate-compatibility-audit-2026-06-07`](../../../analysis/authentigate-compatibility-audit-2026-06-07.md) §6 (F7)
- **Зависит от:** [STORY-IDS-EID-03](STORY-IDS-EID-03-provider-plugin-backbone.md) (`ProviderRuntime` экспонирует `secret_box`)
...
- **Использование провайдером:** `start_flow` пишет `code_verifier_encrypted = secret_box.seal(code_verifier)`, `code_verifier_hash = sha256(code_verifier)`; callback — `secret_box.open(...)`. (Само использование — в [EID-02](../eid-deferred/STORY-IDS-EID-02-real-eid-providers.md); здесь — примитив + слот в `ProviderRuntime`.)

## Вне scope
- Логика PKCE/генерация challenge — адаптер Authentigate ([EID-02](../eid-deferred/STORY-IDS-EID-02-real-eid-providers.md)).
...
- Ключ был удалён в CLEANUP-02: [STORY-IDS-CLEANUP-02 E20](../cleanup/STORY-IDS-CLEANUP-02-placeholders-hardening.md).
...
## Acceptance Criteria
- [ ] Есть `SessionSecretBox` (порт + Fernet-реализация); `seal`/`open` обратимы; round-trip-тест.
- [ ] `EID_SESSION_ENC_KEY` обязателен в `pilot` (понятная `ConfigError` при отсутствии), допустим дефолт в `demo`.
- [ ] `secret_box` доступен провайдерам через `ProviderRuntime`.
- [ ] Ключ шифрования отделён от `eid_secret` (разные назначения).
- [ ] Offline-набор зелёный.
