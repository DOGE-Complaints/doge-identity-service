...
- **Зависит от:** [STORY-IDS-EID-03](STORY-IDS-EID-03-provider-plugin-backbone.md) (`ProviderRuntime` экспонирует тулкит)
...
## Вне scope
- Маппинг claims `id_token` → `EIDVerificationResult` — это адаптер Authentigate ([EID-02](../eid-deferred/STORY-IDS-EID-02-real-eid-providers.md)).
- PKCE `code_verifier` шифрование — [EID-08](STORY-IDS-EID-08-session-secret-box.md).
- Конкретные scope/acr — [EID-04](STORY-IDS-EID-04-provider-owned-config.md).

## Точки в коде (реализовано, pkg-000020)
- Модуль OIDC (provider-agnostic): [`src/core/security/oidc/`](../../../../src/core/security/oidc/) — `discovery.py` (`OidcDiscoveryClient` + per-issuer кэш), `jwks_cache.py` (`JwksCache` TTL + `resolve_key_set_for_kid` refresh-on-unknown-kid), `id_token.py` (`IdTokenValidator` RS256 + `iss/aud/exp/iat/nonce` → `OidcIdTokenValidationError(IDENTITY_VALIDATION_FAILED)`), `toolkit.py` (`OidcToolkit` + `build_oidc_toolkit`).
- Якорь-паттерн `JWTClaimsRegistry` (HS256/симметрично для Supabase): [`supabase_validator.py`](../../../../src/core/auth/supabase_validator.py) — переиспользован подход, OIDC добавил RS256-ветку.
- `joserfc`, `httpx` в зависимостях: [`pyproject.toml`](../../../../pyproject.toml).
- Таймаут OIDC в конфиге: [`schema.py`](../../../../src/core/config/schema.py) (`oidc_request_timeout_s`).
- Проводка: слот `ProviderRuntime.oidc: OidcToolkit | None` ([`runtime.py:21`](../../../../src/core/providers/runtime.py)); `build_oidc_toolkit` только non-mock ([`runtime_factory.py:25`](../../../../src/core/providers/runtime_factory.py)).
- Тесты: [`test_oidc_toolkit.py`](../../../../tests/test_oidc_toolkit.py) (8, `httpx.MockTransport`).
...
- [x] `IdTokenValidator` проверяет подпись RS256 + `iss/aud/exp/iat/nonce`; невалидный токен → доменная ошибка валидации (мапится в `IDENTITY_VALIDATION_FAILED`, см. [EID-05](STORY-IDS-EID-05-canonical-provider-contract.md)).
